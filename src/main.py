...
def find_best_state(states, basis_weights=np.ones(22), gamma=0.9):
    print(len(states))
    Q_value = np.zeros(len(states))

    for possible_state in range(len(states)):
        heights = states[possible_state]["height"]
        bumpiness = states[possible_state]["bumpiness"]
        max_height = states[possible_state]["max_height"]
        holes = states[possible_state]["holes"]
        weight = states[possible_state]["weight"]

        value_sum = 0
        for j in range(len(heights)):
            value_sum += basis_weights[j] * heights[j]
        for j in range(len(bumpiness)):
            value_sum += basis_weights[j] * bumpiness[j]
        value_sum += basis_weights[19] * max_height
        value_sum += basis_weights[20] * holes
        value_sum += basis_weights[21] * weight

        Q_value[possible_state] = (1 / len(states)) * (1 + gamma * value_sum)

    return states[np.argmax(Q_value)]


def make_results_spreadsheet(score, iterations):
    """
...
for i in range(ITERATIONS):
    ...
    if done:
        ...
        if i is not ITERATIONS:
            make_results_spreadsheet(info.get('score'), round_iterations)
        env.reset()
        action_set = []
        action = 0
    env.render()
